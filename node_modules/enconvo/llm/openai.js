const {AIMessage, ChatMessage, HumanMessage, SystemMessage} = require("langchain/schema");
                ...
                this.completionWithRetry({
                    ...
                    onmessage: (event) => {
                        if (event.data?.trim?.() === "[DONE]") {
                            ...
                            if (!resolved &&
                                !rejected &&
                                (message.choices?.length > 0 && message.choices?.every((c) => c.finish_reason != null))) {
                                resolved = true;
                                resolve(response);
                            }
                        }
    ...
    async completionWithRetry(request, options) {
        ...
        if (this.azureOpenAIApiKey) {
            ...
            if (this.clientConfig.organization) {
                this.clientConfig.headers['OpenAI-Organization'] = this.clientConfig.organization;
            }
        ...
        // console.log(JSON.stringify(axiosOptions));
        ...
        makeCompletionRequest = async () => {
            ...
            let params = '';
            for (let key in axiosOptions.params) {
                if (axiosOptions.params.hasOwnProperty(key)) {
                    if (params !== '') {
                        params += '&';
                    }
                    params += encodeURIComponent(key) + '=' + encodeURIComponent(axiosOptions.params[key]);
                }
            }
            ...
            console.log(`axiosOptions.headers: ${JSON.stringify(axiosOptions.headers)}`)
            console.log(`url: ${url}`)
            ...
            const json = await resp.onmessage((value, end) => {
...
class PromptLayerChatOpenAI extends ChatOpenAI {
    ...
    async _generate(messages, options, runManager) {
        ...
        for (let i = 0; i < generatedResponses.generations.length; i += 1) {
            ...
            const promptLayerRespBody = await promptLayerTrackRequest(this.caller, "llm.PromptLayerChatOpenAI", messageDicts, this._identifyingParams(), this.plTags, parsedResp, requestStartTime, requestEndTime, this.promptLayerApiKey);
